...
func (s *DBService) deleteInactiveMembers(tx *sql.Tx, allContacts []int) error {
	// Placeholder slice for the parameterized query
	var params []interface{}
	var placeholders []string

	// Populate the params and placeholders
	for i, contactId := range allContacts {
		params = append(params, contactId)
		placeholders = append(placeholders, fmt.Sprintf("$%d", i+1))
	}

	// Join the placeholders (e.g., $1, $2, $3) for the query
	placeholderStr := strings.Join(placeholders, ",")
	query := fmt.Sprintf(DeleteInactiveMembersQuery, placeholderStr)
...
func (s *DBService) deleteLapsedMember(tx *sql.Tx, contactId int) error {
	_, err := tx.Exec(DeleteLapsedMembersQuery, strconv.Itoa(int(contactId)))
	return err
}
